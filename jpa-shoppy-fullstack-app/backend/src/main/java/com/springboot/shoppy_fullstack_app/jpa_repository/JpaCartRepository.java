...
public interface JpaCartRepository extends JpaRepository<CartItem, Integer> {
    ...
    @Query("""
            select 
                new com.springboot.shoppy_fullstack_app.dto.CartListResponseDto (v.id,v.mname,v.phone,v.email,v.pid,v.name,v.info,v.image,v.price,v.size,v.qty,v.cid,v.totalPrice) 
                from CartListView v 
                where v.id = :id
            """)
